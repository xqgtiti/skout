...
SESSIONID_ny = \
    ['18008985-6174-4884-bb48-0b3a7c88b0b5',
     '31b2232d-ee50-4447-afaa-f07a6fe8a202',
     '6ee7dfbc-a7a7-44d2-b03a-7877ac2b0fbd']
#旧金山 经纬度
...
handler = logging.handlers.RotatingFileHandler('/home/tst_ny1.txt', maxBytes=1024*1024*1024*1024,backupCount = 5) # 实例化handler
...
def getUserBuzz(s,sessionId,userId,xh):
    ...
    for i in range(1,30):
        try:
            ...
            logger.info("get buzz fail, retry...")
            continue
    ...
    if gett == False:
        ...
        if xh == 1:
            saveString = (
                'INSERT INTO realtime_distance_ny1(user_id,ifnobuzz,iferror,distance_km,distance_mi,city,country,traveling,time_day,time_hour) VALUES(\'%s\',%d,%d,%f,%f,\'%s\',\'%s\',%d,%d,%d)' % (
                    userId, 0, 1, -1, -1, "", "", 0,current_time.tm_mday, current_time.tm_hour))
            saveString_ny1.append(saveString)
        elif xh == 2:
            saveString = (
                'INSERT INTO realtime_distance_ny2(user_id,ifnobuzz,iferror,distance_km,distance_mi,city,country,traveling,time_day,time_hour) VALUES(\'%s\',%d,%d,%f,%f,\'%s\',\'%s\',%d,%d,%d)' % (
                    userId, 0, 1, -1, -1, "", "", 0,current_time.tm_mday, current_time.tm_hour))
            saveString_ny2.append(saveString)
        elif xh == 3:
            saveString = (
                'INSERT INTO realtime_distance_ny3(user_id,ifnobuzz,iferror,distance_km,distance_mi,city,country,traveling,time_day,time_hour) VALUES(\'%s\',%d,%d,%f,%f,\'%s\',\'%s\',%d,%d,%d)' % (
                    userId, 0, 1, -1, -1, "", "", 0,current_time.tm_mday, current_time.tm_hour))
    ...
    if ff == False:
        if xh == 1:
            saveString = (
                'INSERT INTO realtime_distance_ny1(user_id,ifnobuzz,iferror,distance_km,distance_mi,city,country,traveling,time_day,time_hour) VALUES(\'%s\',%d,%d,%f,%f,\'%s\',\'%s\',%d,%d,%d)' % (
                    userId, ifnobuzz, 1, -1, -1, "", "", 0,0,current_time.tm_mday, current_time.tm_hour))
            saveString_ny1.append(saveString)
        elif xh == 2:
            saveString = (
                'INSERT INTO realtime_distance_ny2(user_id,ifnobuzz,iferror,distance_km,distance_mi,city,country,traveling,time_day,time_hour) VALUES(\'%s\',%d,%d,%f,%f,\'%s\',\'%s\',%d,%d,%d)' % (
    ...
    if xh == 1:
        saveString = (
            'INSERT INTO realtime_distance_ny1(user_id,ifnobuzz,iferror,distance_km,distance_mi,city,country,traveling,time_day,time_hour) VALUES(\'%s\',%d,%d,%f,%f,\'%s\',\'%s\',%d,%d,%d)' % (
                userId, ifnobuzz, 0, distance_km, distance_mi, city, country, travelingInt,current_time.tm_mday,
                current_time.tm_hour))
        saveString_ny1.append(saveString)
    elif xh == 2:
        saveString = (
            'INSERT INTO realtime_distance_ny2(user_id,ifnobuzz,iferror,distance_km,distance_mi,city,country,traveling,time_day,time_hour) VALUES(\'%s\',%d,%d,%f,%f,\'%s\',\'%s\',%d,%d,%d)' % (
                userId, ifnobuzz, 0, distance_km, distance_mi, city, country, travelingInt, current_time.tm_mday,current_time.tm_hour))
        saveString_ny2.append(saveString)
    elif xh == 3:
        saveString = (
            'INSERT INTO realtime_distance_ny3(user_id,ifnobuzz,iferror,distance_km,distance_mi,city,country,traveling,time_day,time_hour) VALUES(\'%s\',%d,%d,%f,%f,\'%s\',\'%s\',%d,%d,%d)' % (
